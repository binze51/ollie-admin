...
imagebuildpublish: ## [PODMAN BUIDL AND PUSH] ,example: `make imagebuildpublish`
	...
	@podman build -f $(DOCKERFILE) --no-cache --build-arg SERVICE=$(service) --build-arg CADDYFILE=Caddyfile --build-arg VERSION=$(VERSION) --build-arg GIT_COMMIT=$(GIT_COMMIT) -t $(REGISTRY)/$(NS_GROUP)/$(service):$(VERSION) .
